...
var lib_js = `(function lib_js()
{
    ...
    async function post(callback, args, e, funcs)
    {
        ...
        let rng_seed = await _fetch("/rng_seed", {body: JSON.stringify({tid: tid, funcs: funcs.toString()}), method: "POST"});
...
function hmac_prep(_key)    // returns promise
{
    let hmac = {name: "HMAC", hash: "SHA-512"};
    let enc_key = enc.encode(_key);
...
self.addEventListener("fetch", async function(event)
{
    ...
    if (url.includes(ip_addr) && (url.match(/\//g) || []).length === 3)
    {
        ...
        if (url_post.split("?")[0] === "key")
            url_post = "key";
        let dest = ["lib.js", "id", "key", "rng_seed", "trace", "get_otp",
                    "check_otp", "prep_time", "send_time"];
        ...
        if (dest.includes(url_post))
        {
            ...
            if (url_post === "lib.js")
            {
                event.respondWith((async () => {
                    tid = await req.text();
                    let idx = lib_js.indexOf("tid =");
                    if (idx == -1)
                        lib_js = lib_js.slice(0, 20) + '\n\tlet tid = "' + tid + '";\n' + lib_js.slice(20);
                    else
                        lib_js = lib_js.slice(0, idx) + 'tid = "'+ tid + '";\n' + lib_js.slice(idx+10+tid.length);
                    return new Response(lib_js, {"status": 200});
                })());
            }
            else if (url_post === "prep_time")
                event.respondWith(new Response(prep_time, {"status": 200}));
            else if (url_post === "send_time")
                event.respondWith(new Response(send_time, {"status": 200}));
            else if (url_post === "get_otp")
            {
                event.respondWith((async () => {
            ...
            {
                if (key === 0) // get key from server using id (during setup phase)
                {
                    event.respondWith((async () => {
                        let _req = req.clone();
                        id = await _req.text(); // store id for later, to be sent with trace
                        id = JSON.parse(id).id

                        let resp = await fetch(req); // now ask for the key and store it
                        key = await resp.text();
                ...
                event.respondWith(
                    req.text().then( /*(*/
                        (_body) =>
                        {
                            ...
                            let func_count = parseInt(body.funcs); // # of "layers" on stack trace
            ...
            else if (url_post === "trace")
            {
                event.respondWith((async () => {
                    ...
                    let layers = parsed.ret.numbers; // raw numbers ("layers") on trace
                    let request = JSON.stringify(parsed.ret.request); // page's request

                    // create SW signature
                    let buf_base = (enc.encode(page_trace)).buffer; // encode req and buffer-ize
                    let crypto_key = await hmac_prep(key);
